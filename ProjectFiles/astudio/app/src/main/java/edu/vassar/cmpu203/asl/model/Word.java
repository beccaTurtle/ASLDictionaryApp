package edu.vassar.cmpu203.asl.model;
import java.util.*;

/**
 * Representation of an entry in an online translation dictionary between English and ASL
 */
public class Word implements Comparable, java.io.Serializable{
    private String english;
    private int asl;

    private String category;
    private boolean isFavorite = false;

    /**
     * Constructor creates a word
     * @param english : text for the word
     * @param path : location of a gif of its sign
     */
    public Word(String english, int path, String category){
        this.english = english;
        this.asl = path;
        this.category = category;
    }

    /**
     * Changes if the word is favorited or not
     * @param newStatus : updated value for isFavorite
     */
    public void switchFavoriteStatus(boolean newStatus){
        isFavorite = newStatus;
    }

    /**
     * Gets the english text of the word
     * @return String english word
     */
    public String getEnglish(){
        return english;
    }

    /**
     * Gets the path to the asl gif
     * @return String path
     */
    public int getASL(){
        return asl;
    }

    /**
     * Asks if the sign is a favorite
     * @return boolean isFavorite
     */
    public boolean getFavoriteStatus(){
        return isFavorite;
    }

    /**
     * Gets the category
     * @return String category
     */
    public String getCategory(){return category;}


    /**
     * Compares Words by their English strings
     * @param o the object to be compared.
     * @return int comparing the objects
     */
    @Override
    public int compareTo(Object o) {
       if (o instanceof Word) {
           Word other = (Word) o;
           return this.getEnglish().compareTo(other.getEnglish());
       }
       return -1;
    }
}